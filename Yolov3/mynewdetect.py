...
def backtotensor(img0, channel_in=1):  # <--- !! 修改 1: 接收 channel_in
    """
    根据 channel_in 动态转换 tensor
    """
    # 逆向操作1：改变通道位置
    img0 = np.transpose(img0, (2, 0, 1))  # (3, 256, 256)

    if channel_in == 1:
        # 逆向操作2：将三个通道还原回一个通道
        img0 = np.mean(img0, axis=0, keepdims=True)  # (1, 256, 256)

    # (如果 channel_in == 3, img0 保持 [3, 256, 256], 这是正确的)

    # 逆向操作4：将0-255还原回0-1范围
    img0 = img0 / 127.5 - 1
    img0 = torch.tensor(img0, dtype=torch.float32)
    return img0  # 返回 [1, 256, 256] 或 [3, 256, 256]
...
# 一致性随机噪声+平滑
def draw_masks_SB(img, bbox, identities=None, offset=(0, 0), rand_center_x=0.5, rand_center_y=0.5, rand_width=0.5,
                  rand_height=0.5):
    for i, box in enumerate(bbox):
        ...
        img[mask_y1:mask_y2, mask_x1:mask_x2] = mask_img * img[mask_y1:mask_y2, mask_x1:mask_x2] + (
                    1 - mask_img) * noise
...
# 一致性随机纯色+平滑
def draw_masks_darkSB(img, bbox, identities=None, offset=(0, 0), rand_center_x=0.5, rand_center_y=0.5, rand_width=0.5,
                      rand_height=0.5, randlight=0.5, cifar_img=None):
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
...
# 一致性随机噪声
def draw_masks(img, bbox, identities=None, offset=(0, 0), rand_center_x=0.5, rand_center_y=0.5, rand_width=0.5,
               rand_height=0.5):
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
...
# 一致性随机纯色######################################
def draw_masks_dark(img, bbox, identities=None, offset=(0, 0), rand_center_x=0.5, rand_center_y=0.5, rand_width=0.5,
                    rand_height=0.5, randlight=0.5, cifar_img=None):
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
...
def detect(model,
           ...
           device='cpu', save_txt=False, classes=[0],
           agnostic_nms=False,
           cifar_img=None,
    ...
    for img, im0s in dataset:
        ...
        imgt = backtotensor(im0, channel_in=channel_in)  # imgt shape: [C, H, W]
    ...
    stacked_result = torch.stack(tensor_list, dim=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='Yolov3DeepSort/yolov3/weights/yolov3-spp-ultralytics.pt',
                        help='path to weights file')
    parser.add_argument('--source', type=str, default='dataset/ped2/training/frames/01',
